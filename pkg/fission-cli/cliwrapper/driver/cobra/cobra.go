...
import (
	...
	wCli "github.com/fnlize/fnlize/pkg/fission-cli/cliwrapper/cli"
	"github.com/fnlize/fnlize/pkg/fission-cli/cliwrapper/driver/cobra/helptemplate"
	cmd "github.com/fnlize/fnlize/pkg/fission-cli/cmd"
...
func optionalFlags(cmd *cobra.Command, flags ...flag.Flag) {
	for _, f := range flags {
		...
		if f.Deprecated {
			...
			var _ = cmd.Flags().MarkDeprecated(f.Name, usage)
		} else if f.Hidden {
			var _ = cmd.Flags().MarkHidden(f.Name)
		}
	}
}
...
func requiredFlags(cmd *cobra.Command, flags ...flag.Flag) {
	for _, f := range flags {
		toCobraFlag(cmd, f, false)
		var _ = cmd.MarkFlagRequired(f.Name)
	}
}
